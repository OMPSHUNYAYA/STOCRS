...
def make_adversarial_arrivals(rng, valid_nodes, delayed_nodes):
    phase1_valid = [n for n in valid_nodes if n not in delayed_nodes]
    arrivals = rng.sample(phase1_valid, len(phase1_valid))

    duplicate_targets = rng.sample(phase1_valid, min(4, len(phase1_valid)))
    duplicate_tokens = duplicate_targets[:]

    invalid_tokens = [
        "INJECT:FAKE1=999",
        "INJECT:X1=999",
        "INJECT:Z1=12345",
        "INJECT:SHADOW_NODE=42"
    ]
    rng.shuffle(invalid_tokens)

    mixed = arrivals + duplicate_tokens + invalid_tokens[:3]
    rng.shuffle(mixed)
    return mixed
...
def sanitize_arrivals(arrivals, program):
    ...
    return accepted, duplicate_count, invalid_count
...
def run_system(system_name, program, raw_phase1_arrivals, raw_phase2_arrivals,
    ...
    clean_phase1_arrivals, dup1, invalid1 = sanitize_arrivals(raw_phase1_arrivals, program)
    clean_phase2_arrivals, dup2, invalid2 = sanitize_arrivals(raw_phase2_arrivals, program)
...
def single_run(seed, systems_count):
    ...
    for i in range(systems_count):
        sys_rng = random.Random(seed * 100 + i * 17 + 3)

        raw_phase1 = make_adversarial_arrivals(sys_rng, valid_nodes, delayed_nodes)

        phase2_valid = delayed_nodes[:]
        phase2_dups = [delayed_nodes[0]]
        phase2_invalid = [f"INJECT:BADZ{i}=777"]
        raw_phase2 = phase2_valid + phase2_dups + phase2_invalid
        ...
        system = run_system(
            ...
            anchor_s=400000.0 + i * 211.0,
            elapsed_p1_s=86400.0 * (150.0 + i),
            elapsed_p2_s=86400.0 * (151.0 + i),
    ...
    all_noise_detected = all(s["duplicate_count"] > 0 and s["invalid_count"] > 0 for s in systems)
    ...
    certificate = hash_obj({
        ...
        "all_noise_detected": all_noise_detected,
...
def main():
    ...
    if args.systems < 3:
        raise ValueError("Use at least 3 systems for the adversarial multi-system demo.")

    if args.stress <= 1:
        result = single_run(args.seed, args.systems)

        print("STOCRS v6 Adversarial Demo")
        ...
        print("Adversarial Noise Detected:", "YES" if result["all_noise_detected"] else "NO")
    ...
    print("STOCRS v6 Adversarial Stress Test")
