...
def initial_fragment_for_system(index):
    # Intentionally incomplete fragments:
    # each includes at least one downstream node whose prerequisites are missing
    fragments = [
        ["X1", "X2", "A1", "B1", "C1", "D1"],
        ["X2", "X3", "A2", "B2", "C2", "D2"],
        ["X3", "X4", "A3", "B3", "C3", "E1"],
...
def partial_share_union(current_nodes, incoming_nodes, cap):
    merged = []
    seen = set()
...
def single_run(seed, systems_count):
    ...
    for i in range(systems_count):
        name = f"S{i+1}"
        fragment = initial_fragment_for_system(i)[:]
        rng.shuffle(fragment)
        snapshot = run_system(
            name,
            program,
            known_nodes=fragment,
            anchor_s=600000.0 + i * 191.0,
            elapsed_s=86400.0 * (200.0 + i),
            drift_ppm=drift_list[i % len(drift_list)],
        )
        phase1.append(snapshot)

    # Phase 2: pairwise partial sharing
    phase2 = []
    for i in range(systems_count):
        left = phase1[i]["known_nodes"]
        right = phase1[(i + 1) % systems_count]["known_nodes"]
        shared = partial_share_union(left, right, cap=9)
        snapshot = run_system(
            f"S{i+1}",
            program,
            known_nodes=shared,
            anchor_s=600000.0 + i * 191.0,
            elapsed_s=86400.0 * (201.0 + i),
            drift_ppm=drift_list[i % len(drift_list)],
        )
        phase2.append(snapshot)

    # Phase 3: wider partial sharing, still capped below full graph
    phase3 = []
    for i in range(systems_count):
        a = phase2[i]["known_nodes"]
        b = phase2[(i + 2) % systems_count]["known_nodes"]
        merged = partial_share_union(a, b, cap=14)
        snapshot = run_system(
            f"S{i+1}",
            program,
            known_nodes=merged,
            anchor_s=600000.0 + i * 191.0,
            elapsed_s=86400.0 * (202.0 + i),
            drift_ppm=drift_list[i % len(drift_list)],
        )
        phase3.append(snapshot)

    # Phase 4: full reconvergence
    phase4 = []
    for i in range(systems_count):
        ...
        snapshot = run_system(
            ...
            anchor_s=600000.0 + i * 191.0,
            elapsed_s=86400.0 * (203.0 + i),
            drift_ppm=drift_list[i % len(drift_list)],
        )
        phase4.append(snapshot)
    ...
    phase4_complete_ok = all(s["unresolved"] == [] for s in phase4)
    ref_values = phase4[0]["values"]
    phase4_match_ok = all(s["values"] == ref_values for s in phase4)

    local_times = [s["local_time_s"] for s in phase4]
    ...
    certificate = hash_obj({
        ...
        "phase4_complete_ok": phase4_complete_ok,
        "phase4_match_ok": phase4_match_ok,
...
def main():
    ...
    if args.systems < 3:
        raise ValueError("Use at least 3 systems for the partial-sharing demo.")

    if args.stress <= 1:
        result = single_run(args.seed, args.systems)

        print("STOCRS v8 Partial Sharing Demo")
        ...
        print("Phase 4 Complete OK:", "YES" if result["phase4_complete_ok"] else "NO")
        print("Phase 4 Match OK:", "YES" if result["phase4_match_ok"] else "NO")
        ...
        for i in range(result["systems_count"]):
            ...
            print(f"{s1['system']} Phase4 Local Time:", s4["local_time_s"])

        print("Final Node Count:", len(result["phase4"][0]["values"]))
        print("Final E1:", result["phase4"][0]["values"]["E1"])
    ...
    for i in range(args.stress):
        ...
        passed = (
            ...
            result["phase4_complete_ok"] and
            result["phase4_match_ok"]
        )
    ...
    print("STOCRS v8 Partial Sharing Stress Test")
