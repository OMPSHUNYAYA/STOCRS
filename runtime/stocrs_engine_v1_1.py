...
def choose_claim_value(values):
    ...
    for value in values:
        ...
        if key not in normalized:
            normalized[key] = {"value": value, "count": 0}
        normalized[key]["count"] += 1

    if len(normalized) == 1:
        item = next(iter(normalized.values()))
        return {
            "status": "accepted",
            "value": item["value"],
            "support": item["count"],
    ...
    return {
        "status": "conflict",
        "value": None,
        "support": max(item["count"] for item in normalized.values()),
        "distinct_values": len(normalized),
    }
...
def print_conflict_demo(result):
    ...
    print("Recovery Run")
    print(f"Resolved Values: {result['recovery_run']['values']}")
    print(f"Unresolved: {result['recovery_run']['unresolved']}")
    print(f"Conflicts: {result['recovery_run']['conflicts']}")
    print()
